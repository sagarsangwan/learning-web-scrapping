...
def upload_to_mysql(info):
    ...
    for n in info:
        ...
        if n.get("name") in data:
            ...
            cur.execute(
                "INSERT INTO question_paper(qp_name, link, year) values(%s, %s, %s)",
                (str((n.get("name"))), str(n.get("link")), str(n.get("year"))),
            )
            cur.execute("SELECT LAST_INSERT_ID();")
            id = cur.fetchone()[0]
            cur.execute(
                "INSERT INTO subjects(subject_name, subject_desc, question_papers) values(%s, %s, %s)",
                (str((n.get("name"))), str((n.get("desc"))), str(id)),
            )
            cur.execute("SELECT LAST_INSERT_ID();")
            sub_id = cur.fetchone()[0]
            subject_ids.append(str(sub_id))
            cur.execute(
                "INSERT INTO subjects_to_course(subject_id, course_ids) values(%s, %s)",
                (str(sub_id), "1"),
            )
    ...
    if len(subject_ids) > 0:
        cur.execute(
            "INSERT INTO course_to_subjets(subjects_ids, course_id) VALUES (%s, %s) ON DUPLICATE KEY UPDATE subjects_ids=subjects_ids+%s",
            (",".join(subject_ids), "1", ",".join(subject_ids)),
        )
...
# cur = mydb.cursor()
# cur.execute("INSERT INTO course_to_subjets ")
# cur.execute("SELECT * FROM course_to_subjets")
# data = cur.fetchall()
# print(data)
# cur.close()
