...
def upload_to_mysql(info):
    ...
    for n in info:
        ...
        if n.get("name") in data:
            ...
            cur.execute("INSERT INTO subjects_to_course(subject_id, course_ids) values(%s, %s)",(str(sub_id), "3"))
            print("inserted-", n.get("name"))

    if len(subject_ids) > 0: 
        cur.execute("INSERT INTO course_to_subjets(subjects_ids, course_id) VALUES (%s, %s) ON DUPLICATE KEY UPDATE subjects_ids=subjects_ids+%s", (",".join(subject_ids), "3", ",".join(subject_ids)))
